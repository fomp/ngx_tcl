...
#
# RCS: @(#) $Id: upload.tcl 6157 2011-02-17 15:42:17Z dino $

package provide httpd::upload 0.9
...
proc UploadDomain {dir cmd maxfiles maxbytes totalbytes unique sock suffix} {
    ...
    # patch: worsch "-trans auto" does not work here (if in thread mode?)
    fconfigure $sock -trans crlf 
...
proc UploadFindBoundary {sock} {
	...
	if {[regexp ^--$upload(boundary) $line]} {
	    fileevent $sock readable [list UploadReadHeader $sock]
            ...
            Log $sock "" "UploadFindBoundary Unexpected line $line"
	}
    }
}
...
proc UploadDone {sock} {
	...
	switch -- [lindex $ec 0] {
        ...
        hserver::log::logE HSRV ERROR- ERROR $result ERRORINFO $::errorInfo "error occured"

        Httpd_Error $sock 500
